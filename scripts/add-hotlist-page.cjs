...
const layoutPath = 'src/frontend/src/components/Layout.tsx';
let layout = fs.readFileSync(layoutPath, 'utf8');
layout = layout.replace('import { Home, RefreshCw, Search, Star, Users } from "lucide-react";', 'import { Flame, Home, RefreshCw, Search, Users } from "lucide-react";');
layout = layout.replace('{ to: "/favorites", label: "Favoritter", icon: Star, ocid: "nav-favoritter" },', '{ to: "/favorites", label: "Hotlist", icon: Flame, ocid: "nav-hotlist" },');
fs.writeFileSync(layoutPath, layout);

const favoritesPath = 'src/frontend/src/pages/FavoritesPage.tsx';
const hotlistPage = String.raw`import { cn } from "@/lib/utils";
import { Flame, Search, Shield, Target, Trophy } from "lucide-react";
  ...
  return (
    <div className="rounded-xl bg-background/70 border border-border px-3 py-3">
      <div className="flex items-center gap-2 text-primary mb-1">
        ...
        <span className="text-[9px] uppercase tracking-widest font-display font-bold">
          {label}
        </span>
      </div>
    ...
    <div className="space-y-5" data-ocid="hotlist-page">
      <section className="rounded-2xl bg-card border border-border p-4 space-y-4">
        ...
        <div className="grid grid-cols-3 gap-2">
          <HotStat icon={Trophy} label="Topp" value={topPlayer ? topPlayer.name.split(" ").at(-1) ?? "-" : "-"} />
          <HotStat icon={Target} label="MEP sist" value={topInsight?.latestMep?.toFixed(1) ?? "-"} />
          <HotStat icon={Shield} label="Spillere" value={players.length.toString()} />
        </div>
      </section>

      <div className="flex items-center gap-2 overflow-x-auto scrollbar-none" data-ocid="hotlist-filter-pills">
...
`;
fs.writeFileSync(favoritesPath, hotlistPage);
